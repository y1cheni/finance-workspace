  ...
  return (
    <div className="mb-3">
      ...
      <input type="number" value={value} step={10000}
        onChange={e => onChange(Number(e.target.value))}
        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1"
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <aside className="lg:w-68 shrink-0">
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">資產負債（現況）</p>
            ...
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">每月收支</p>
            ...
            <hr className="my-3 border-gray-100" />
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">成長假設（年化）</p>
        ...
        <div className="flex-1 space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: '目前淨值',             value: fmt(today.netWorth),                 primary: true  },
              { label: `${projYears} 年後淨值`, value: fmt(final.netWorth),                primary: false },
              { label: '目前儲蓄率',           value: `${today.savingsRate.toFixed(1)}%`,  primary: false },
              { label: '目前負債比',           value: `${today.debtToAssets.toFixed(1)}%`, primary: false },
            ].map(m => (
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <p className="text-sm font-medium text-gray-700 mb-4">資產結構與淨值（B/S）</p>
            <ResponsiveContainer width="100%" height={320}>
              <BarChart data={bsChartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                ...
                {Object.keys(CHART_COLORS).map(k => (
                  <Bar key={k} dataKey={k} stackId="a" fill={CHART_COLORS[k]} />
                ))}
                <Line type="monotone" dataKey="淨值" stroke="#374151" strokeWidth={2} dot={false} />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <p className="text-sm font-medium text-gray-700 mb-4">損益表（P/L）趨勢</p>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={plChartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                ...
                <Tooltip formatter={(v: any) => fmt(Number(v))}
                  contentStyle={{ border: '1px solid #F3F4F6', borderRadius: 8, fontSize: 12 }} />
                <Legend wrapperStyle={{ fontSize: 12 }} />
                <Line type="monotone" dataKey="總收入" stroke={PRIMARY} strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="總支出" stroke="#D1D5DB" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="淨收入" stroke="#374151" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
              ...
              {(['bs', 'pl'] as const).map(t => (
                <button key={t} onClick={() => setActiveTab(t)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                    activeTab === t ? 'text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                  }`}
                  style={activeTab === t ? { backgroundColor: PRIMARY } : {}}
                  ...
                  <tbody>
                    {records.map(r => (
                      <tr key={r.year} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-1.5 pr-3 font-medium text-gray-700">{r.year}</td>
                        <td className="py-1.5 pr-3 text-gray-600">{fmt(r.cash)}</td>
                        <td className="py-1.5 pr-3 text-gray-600">{fmt(r.investments)}</td>
                        <td className="py-1.5 pr-3 text-gray-600">{fmt(r.realEstate)}</td>
                        <td className="py-1.5 pr-3 font-medium text-gray-700">{fmt(r.totalAssets)}</td>
                        <td className="py-1.5 pr-3 text-gray-500">{fmt(r.liabilities)}</td>
                        <td className="py-1.5 pr-3 font-medium" style={{ color: PRIMARY }}>{fmt(r.netWorth)}</td>
                        <td className="py-1.5 text-gray-500">{r.debtToAssets}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {records.map(r => (
                      <tr key={r.year} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-1.5 pr-3 font-medium text-gray-700">{r.year}</td>
                        <td className="py-1.5 pr-3 text-gray-600">{fmt(r.annualIncome)}</td>
                        <td className="py-1.5 pr-3 text-gray-500">{fmt(r.investmentIncome)}</td>
                        <td className="py-1.5 pr-3 text-gray-600">{fmt(r.totalIncome)}</td>
                        <td className="py-1.5 pr-3 text-gray-500">{fmt(r.annualExpenses)}</td>
                        <td className="py-1.5 pr-3 font-medium" style={{ color: PRIMARY }}>{fmt(r.netIncome)}</td>
                        <td className="py-1.5 text-gray-500">{r.savingsRate}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
